...
bot(
  ...
  async (message, match) => {
    ...
    if (!match) return await message.send('Example : setsudo 923461280347 | mention | reply to msg')
    try {
...
bot(
  ...
  async (message, match) => {
    match = jidToNum(message.reply_message.jid || message.mention[0] || match)
    if (!match) return await message.send('Example : delsudo 923461280347 | mention | reply to msg')
    try {
      const vars = await getVars(message.id)
